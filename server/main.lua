...
QBCore.Functions.CreateCallback("garbagejob:server:NewShift", function(source, cb, continue)
    ...
    if CanPay(Player) or continue then
        ...
        TriggerClientEvent('QBCore:Notify', source, Lang:t("error.not_enough", {
...
RegisterNetEvent('garbagejob:server:PayShift', function(continue)
    ...
    if Routes[CitizenId] then
        ...
        local payoutDeposit = Lang:t("info.payout_deposit", {
            value = depositPay
        })
...
QBCore.Commands.Add("cleargarbroutes", "Removes garbo routes for user (admin only)", {
    ...
    TriggerClientEvent('QBCore:Notify', source, Lang:t("success.clear_routes", {
        value = count
    }), "success")
